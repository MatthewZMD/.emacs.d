...
**** Prerequisite:
     Read [[https://github.com/Sarcasm/irony-mode#irony-server-prerequisites][Irony Server Prerequisites]] and download [[https://cmake.org/download/][CMake]] >= 2.8.3 and [[http://clang.llvm.org/doxygen/group__CINDEX.html][libclang]].
    #+BEGIN_SRC emacs-lisp
    (use-package irony
      :ensure t
      :config
